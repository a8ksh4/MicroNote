...
def process_next(sm=None):
    ...
    clock = time.ticks_ms()
    print(f"PROCESS_NEXT arg: {sm}, {sm.__class__}")

    if not isinstance(sm, PIO):
        # This was called from a timer
        buttons_pressed = PENDING_BUTTONS
    else:
        # This was called from the SM irq
        pin_states = sm.state_machine(0).get()
        buttons_pressed = pins_to_buttons(pin_states)
        buttons_pressed = [PINS.index(b) for b in buttons_pressed]
        print(buttons_pressed)
        # TIMER = Timer(mode=Timer.ONE_SHOT, 
        #                     period=HOLDTIME, 
        #                     callback=process_next)
        # if not TIMER:
        ...
        print(TIMER)
        if not TIMER:
            TIMER = Timer(mode=Timer.PERIODIC, callback=process_next, freq=10)
    ...
    if PENDING_BUTTONS:
        ...
        if ( len(PENDING_BUTTONS) > len(buttons_pressed) 
            ...
            print(tap, len(PENDING_BUTTONS), len(buttons_pressed), clock, TICKER, clock-TICKER)
    ...
    if not EVENTS or not [e for e in EVENTS if not e.active]:
        TIMER.deinit()
        TIMER = None
